...
import (
	...
	"task_manager_app/internal/model"
)
...
func newProject(db *gorm.DB, opts ...gen.DOOption) project {
	...
	_project.User = projectBelongsToUser{
...
type projectBelongsToUser struct {
	db *gorm.DB

	field.RelationField
}

func (a projectBelongsToUser) Where(conds ...field.Expr) *projectBelongsToUser {
	if len(conds) == 0 {
		return &a
	}
...
func (a projectBelongsToUser) WithContext(ctx context.Context) *projectBelongsToUser {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a projectBelongsToUser) Session(session *gorm.Session) *projectBelongsToUser {
	a.db = a.db.Session(session)
	return &a
}

func (a projectBelongsToUser) Model(m *model.Project) *projectBelongsToUserTx {
	return &projectBelongsToUserTx{a.db.Model(m).Association(a.Name())}
}

type projectBelongsToUserTx struct{ tx *gorm.Association }

func (a projectBelongsToUserTx) Find() (result *model.User, err error) {
	return result, a.tx.Find(&result)
}

func (a projectBelongsToUserTx) Append(values ...*model.User) (err error) {
...
func (a projectBelongsToUserTx) Replace(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a projectBelongsToUserTx) Delete(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a projectBelongsToUserTx) Clear() error {
	return a.tx.Clear()
}

func (a projectBelongsToUserTx) Count() int64 {
	return a.tx.Count()
}
